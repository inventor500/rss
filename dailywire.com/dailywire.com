...
simulate-browser --regex '<(?:[hH][0-9]|p)>.*?<a href=".*?data-saferedirecturl=".*?".*?</a>(?:\n|.)*?</(?:[hH][0-9]|p)>' '' '(?:<(?:h[0-9]|p|strong)>|(?:<[aA].*?>)?<(?:strong|b)>).*?(?:WATCH|RELATED|[0-9]{1,2}[%\$] OFF).*?(</(?:strong|b)>)?(?:</[aA]>)?</(?:h[0-9]|p)>' '' '<[pP]>.*?CHECK OUT THE DAILY WIRE HOLIDAY GIFT GUIDE.*?</[pP]>' '' https://www.dailywire.com/feeds/rss.xml
